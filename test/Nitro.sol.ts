...
import type { Nitro } from "../typechain/Nitro";
import { ethers } from "hardhat";

chai.use(solidity);
const { expect, assert } = chai;

describe("Nitro", async function () {
  it("should have 18 decimals", async () => {
    const signers = await ethers.getSigners();

    const alice = signers[0];

    const pgenFactory = await ethers.getContractFactory("Nitro");
    const pgen = (await pgenFactory.deploy(alice.address)) as Nitro;
    await pgen.deployed();
  ...
  it("should have 1 billion supply", async function () {
    ...
    const pgenFactory = await ethers.getContractFactory("Nitro");
    const pgen = (await pgenFactory.deploy(bob.address)) as Nitro;
    await pgen.deployed();
